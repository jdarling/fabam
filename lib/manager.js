...
const getTransport = (requested, options)=>{
  if(typeof(requested)==='function'){
    return new requested(options);
  }
  if(typeof(requested)==='object'){
  ...
  if(!requested){
    return new Transports.IPC(options);
  }
  const reRequested = new RegExp(`^${requested||''}$`, 'i');
  const transport = TransportNames.find((n)=>reRequested.exec(n)) || 'IPC';
  return new Transports[transport](options);
};
...
const registerIpc = (()=>{
  const transports = {};
  return (manager, scheduler, options, ready)=>{
    const tOptions = Object.assign({}, options, {manager, scheduler, basepath: manager.basepath});
    const transport = transports.IPC || (transports.IPC = new Transports.IPC(tOptions));
    const done = ()=>{
      return ready(null, scheduler);
    };
    const createInstance = (ready)=>{
      transport.getInstance(tOptions, ready);
    };
    const createInstances = (toStart, callback)=>{
      if(toStart > 0){
        return createInstance((err, child)=>{
          createInstances(toStart-1, callback);
        });
      }
      return callback();
    };
    createInstances(options.numInstances, done);
  };
})();
